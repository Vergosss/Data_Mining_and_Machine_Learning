...
print('-------------------\n',df)
#df_new = df.groupby('Entity').tail(1).drop(['Entity','Continent','Date'],axis=1)#afairese oles tis object-mh arithmitikes stiles
#plt.figure(figsize=(12,8))#dimiourgia figure me figure size 12x8
#sns.heatmap(df_new.corr(),annot=True,cmap=plt.cm.Reds)#annot: an einai true grapse ti timi tou dedomenou sto keli,cmap:onoma tou colormap
#plt.show()#emfanisi tis eikonas
...
#dimiourgia mitroou sysxetisis metaxy ton xaraktiristikon tou dataset kai apeikonish tou se heatmap-symmetriko mitroo
...
#xoris to group by tha evgazei ti teleytaia grammi tou dataset
...
df_temp = df.loc[df['Date']>'2020-03-19'] # se olo to dataset oles oi pleiades pano apo tin imeromhnia ayth
print(df_temp)#mono oi eggrafes pano apo tin imerominia pou grafo
countries = ['Albania','Algeria','Slovenia','South Africa']#opoies xores thelo
...
df_last = df_last.drop('Continent',axis=1) #afou pirame tis hpeirous tis afairoume apo to dataset
print(continents) #typose tes
print(df_last.columns)
##
#apo th teleytaia grammi kathe xoras xoris tis hpeirous-dates-xora (neo dataset) -
#gia kathe sthlh pou exei meinei(gia na plotaro kathe metavliti pou exei meinei)
