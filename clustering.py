...
#    'Deaths'])  # omadopoihsh vash xoras kai ypologizei statistika gia kathe attribute mono gia thn xora ayth
...
#missing_cases_per_country = df.groupby('Entity')['Cases'].apply(
#    lambda x: x.isnull().sum())  # poses times leipoun apo kathe xora
...
#missing_daily_tests_per_country = df.groupby('Entity')['Daily tests'].apply(lambda x: x.isnull().sum())
#print('-------------------\nMissing values now per country:\n', missing_deaths_per_country, missing_cases_per_country,
#      missing_daily_tests_per_country)
#print('-------------------\n', df)
...
#boxplot_attributes = ['Latitude', 'Longitude', 'Average temperature per year', 'Hospital beds per 1000 people',
...
#
'''
for column in df.drop(['Entity', 'Continent', 'Date', 'Daily tests', 'Cases', 'Deaths'],
                      axis=1):  # rixno aytes pou leo kato giati den tis vazo
    plt.figure()
...
'''
df.drop(['Entity', 'Continent', 'Date', 'Daily tests', 'Cases', 'Deaths'], axis=1).drop_duplicates().hist(bins=15,
                                                                                                          figsize=(
                                                                                                          16, 9),
                                                                                                          rwidth=0.8)
plt.show()
df.drop(['Entity', 'Continent', 'Date', 'Daily tests', 'Cases', 'Deaths'], axis=1).hist(bins=15, figsize=(16, 9),
                                                                                        rwidth=0.8)
...
for output_variable in ['Cases', 'Deaths']:  # kai gia ta krousmata kai tous thanatous
    fig, ax = plt.subplots(
        figsize=(10, 6))  # h subplot epistrefei ena Figure kai enan Axes. Ousiastika dio orismata rows-columns-10x6
    ...
    for key, grp in df_temp[df_temp['Entity'].isin(countries)].groupby(['Entity']):
        ax = grp.plot(ax=ax, kind='line', x='Date', y=output_variable,
                      label=key)  # kind: line -to eidos tis grafikis me grammes
    ...
    plt.legend(
        loc='best')  # to legend sth pyplot perigrafei se ena mikro plaisio kathe diaforetiko xroma/info kathe grammhs
    # to location=best vazei to legend ekei pou yperkalypttei ligotero th grafikh parastash
    plt.xticks(
        rotation=90)  # gyrizo 90 mires tis times tou aksona x(einai imerominies opote na mhn peftoun i mia sthn allh)
...
for column in df_last.columns:
    fig, ax = plt.subplots(ncols=2, figsize=(14,
                                             4))  # ftiakse ena figure kai axes 14x4-gia cols=2 afou 2 einai ta subplots ara tha paei aristera-			#>deksia. an evaza nrows=2 tha tane pano pros kato. ara epistrefei 2 axes
    df_last.plot.scatter(x=column, y='Cases', ax=ax[
        0])  # kane plot scatter me orizontio aksona tin antistixi stili-kai katakoryfo Cases-1os axes
    df_last.plot.scatter(x=column, y='Deaths', ax=ax[
        1])  # kane plot scatter me orizontio aksona tin antistixi stili-kai katakoryfo deaths-2os axes
    # se grouped by dataframe kano .plot.scatter
    plt.show()
    if column == 'Continent':
        fig.autofmt_xdate(
            rotation=90)  # an eisai stin hpeiro gyrna tis times tou katheta epeidh einai onomata gia na xorane
'''
